...
```sql avg_latency
SELECT ROUND(SUM(EPOCH_MS(completed_at - started_at)) / 1000.0 / COUNT(DISTINCT tick_id), 1) AS avg_latency_s
FROM agent_logs
```
...
SELECT
    ...
    MIN(al.started_at) AS tick_started,
    ROUND(SUM(EPOCH_MS(al.completed_at - al.started_at)) / 1000.0, 1) AS pipeline_latency_s,
