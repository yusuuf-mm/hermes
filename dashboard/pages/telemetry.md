...
```sql avg_latency
SELECT ROUND(SUM((cast(completed_at as bigint) - cast(started_at as bigint))) / 1000.0 / COUNT(DISTINCT tick_id), 1) AS avg_latency_s
FROM agent_logs
```
...
SELECT
    agent_name,
    ROUND(AVG((cast(completed_at as bigint) - cast(started_at as bigint))), 0) AS avg_latency_ms
FROM agent_logs
GROUP BY agent_name
...
SELECT
    ...
    agent_name,
    llm_model AS model,
    ROUND((cast(completed_at as bigint) - cast(started_at as bigint)), 0) AS latency_ms,
...
SELECT
    ...
    MIN(al.started_at) AS tick_started,
    ROUND(SUM((cast(al.completed_at as bigint) - cast(al.started_at as bigint))) / 1000.0, 1) AS pipeline_latency_s,
